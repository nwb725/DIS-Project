...
def insert_produce(produce: Produce):
    ...
    db_cursor.execute(
        sql,
        (produce.category, produce.item, produce.unit, produce.variety, produce.price),
    )
    conn.commit()
...
def get_produce_by_filters(
    category=None, item=None, variety=None, farmer_pk=None, farmer_name=None, price=None
):
    sql = """
    ...
    produce = (
        [Produce(res) for res in db_cursor.fetchall()] if db_cursor.rowcount > 0 else []
    )
    return produce
...
def get_orders_by_customer_pk(pk):
    ...
    db_cursor.execute(sql, (pk,))
    orders = (
        [ProduceOrder(res) for res in db_cursor.fetchall()]
...
# Attack, Defense and Speed - Pokemon
# Strength, Durability, Speed - Sport

""" 
SELECT sport_name
FROM Sports
WHERE speed > (
    SELECT pokemon_speed
    FROM Pokemons
