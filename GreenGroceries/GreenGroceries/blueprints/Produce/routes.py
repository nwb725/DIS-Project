...
from GreenGroceries.queries import (
    ...
    Sell,
    insert_sell,
    get_all_produce_by_farmer,
    get_produce_by_filters,
    insert_produce_order,
    update_sell,
...
def produce():
    ...
    if request.method == "POST":
        produce = get_produce_by_filters(
    ...
    return render_template(
        "pages/produce.html", produce=produce, form=form, title=title
    )
...
def add_produce():
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            sell = Sell(
                dict(
                    farmer_pk=current_user.pk, produce_pk=new_produce_pk, available=True
                )
            )
            insert_sell(sell)
...
def buy_produce(pk):
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            update_sell(
                available=False, produce_pk=produce.pk, farmer_pk=produce.farmer_pk
            )
...
def restock_produce(pk):
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            update_sell(
                available=True, produce_pk=produce.pk, farmer_pk=produce.farmer_pk
            )
